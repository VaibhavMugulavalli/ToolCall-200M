...
class CausalSelfAttention(nn.Module):
    def __init__(self, config: ModelConfig) -> None:
        ...
        self.num_kv_heads = config.resolved_num_kv_heads
        self.query_groups_per_kv_head = self.num_heads // self.num_kv_heads
        self.head_dimension = config.hidden_size // config.num_heads
        self.dropout = config.dropout
        self.uses_grouped_query_attention = self.num_kv_heads != self.num_heads
        if self.uses_grouped_query_attention:
            kv_dimension = self.num_kv_heads * self.head_dimension
            self.query_projection = nn.Linear(
                config.hidden_size, config.hidden_size, bias=False
            )
            self.key_projection = nn.Linear(
                config.hidden_size, kv_dimension, bias=False
            )
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        if self.uses_grouped_query_attention:
            key = key.repeat_interleave(self.query_groups_per_kv_head, dim=1)
            value = value.repeat_interleave(self.query_groups_per_kv_head, dim=1)
...
class FeedForward(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        if self.activation == "swiglu":
            hidden = F.silu(self.gate_projection(inputs)) * self.up_projection(inputs)
        else:
