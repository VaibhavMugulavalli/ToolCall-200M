...
from .config import ExperimentConfig, ModelConfig, resolve_project_path
from .model import ToolCallLanguageModel
...
class Trainer:
    ...
    def resume(self, checkpoint: str | Path | None) -> Path | None:
        ...
        # Load checkpoint tensors on CPU first. Mapping the entire checkpoint to
        # CUDA also moves the saved CPU RNG ByteTensor, but torch.set_rng_state()
        # requires that tensor to remain on CPU. Model and optimizer loaders move
        # their own state onto the parameter device during restoration.
        state = torch.load(path, map_location="cpu", weights_only=False)
        saved_model_config = state.get("config", {}).get("model")
        if not isinstance(saved_model_config, dict):
            raise RuntimeError("Checkpoint does not contain a valid model configuration")
        if ModelConfig.from_dict(saved_model_config) != self.config.model:
    ...
    def train(
        ...
        if session_token_budget is not None and session_token_budget <= 0:
        ...
        session_start_tokens = self.tokens_seen

        try:
            while self.step < self.max_steps:
                pause_reason: str | None = None
                if (
                    session_token_budget is not None
                    and self.tokens_seen - session_start_tokens
                    >= session_token_budget
                ):
                    pause_reason = "session_token_budget"
                if (
                    max_session_minutes is not None
                    and (time.perf_counter() - session_started) / 60.0
                    >= max_session_minutes
                ):
                    pause_reason = "max_session_minutes"
                    ...
                    self.metrics.log(
                        {
                            ...
                            "step": self.step,
                            "tokens_seen": self.tokens_seen,
                            "session_tokens": self.tokens_seen - session_start_tokens,
                        }
                    )
                    self._write_summary("paused", last_loss, last_validation)
                    print(
                        f"Session paused at step={self.step:,}, "
