#!/usr/bin/env python3
"""Shared deterministic tokenizer/source utilities retained from scaling."""
...
class OpenAPICatalog:
    ...
    def __init__(self, cache_dir: Path, seed: int, revision: str = "main") -> None:
        self.root = cache_dir / "openapi-directory"
        self.seed = seed
        self.requested_revision = revision
        self._ensure_repository(revision)
    ...
    def _ensure_repository(self, revision: str) -> None:
        if (self.root / ".git").is_dir():
            current = subprocess.check_output(
                ["git", "rev-parse", "HEAD"], cwd=self.root, text=True
            ).strip()
            if revision in {"main", current}:
                return
            subprocess.run(
...
def open_sources(config: dict[str, Any], cache_dir: Path) -> list[SourceState]:
    ...
    openapi_catalog = (
        OpenAPICatalog(
            cache_dir,
            int(config["seed"]),
            str(config.get("openapi_revision", "main")),
        )
        if any(
...
def main() -> None:
    ...
    if config.get("capacity_policy", {}).get("requires_capacity_audit"):
        raise RuntimeError(
            "The final 4B configuration is audit-gated. Run "
            "scripts/audit_source_capacity.py and merge_capacity_reports.py first. "
            "Stage 2 will freeze approved exact allocations and provide the "
            "restart-safe corpus writer; this legacy monolithic writer is disabled "
            "for toolcall_4b.json."
        )
